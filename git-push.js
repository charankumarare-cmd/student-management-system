...
async function main() {
  if (!token) {
    console.error('ERROR_TOKEN_REQUIRED: GitHub Personal Access Token (PAT) is required to push to GitHub.');
    process.exit(1);
  }
  ...
  function getFiles(dirPath) {
    ...
    list.forEach(file => {
      if (file === 'node_modules' || file === '.git' || file === 'dist' || file === 'node') return;
